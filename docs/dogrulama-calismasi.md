# GenCP doğrulama çalışması: README'den taşınan tam metin

Aşağıdaki metin, kök `README.md` dosyasının doğrulama çalışmasına ait bölümleridir (Araştırma kaydı,
Öne çıkanlar, Nereden başlamalı, Ne bulundu, Türkiye'ye genelleme, yeniden eğitim, ODTÜ paketi,
Güncel durum, Kullanılan veri, Nasıl çalıştırılır, Hızlı sözlük, Çalışma disiplini) ve 3 Eylül 2026'da
(WP27) olduğu gibi buraya taşınmıştır. Metne dokunulmamıştır; yalnızca göreli bağlantı ve görsel
yollarının başına, bu dosya `docs/` dizininde bulunduğu için, `../` eklenmiştir. “Güncel durum”
bölümünün tarihi taşındığı günkü hâlidir. Depo bakım kuralları [MAINTAINERS.md](../MAINTAINERS.md)
dosyasındadır.
